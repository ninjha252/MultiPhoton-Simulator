...
function generatePlot(failureRange, fig)
    ...
    TOTAL_ROUNDS = 10000; % Rounds per network
   ...
   fileName = "T-" + trustedNodeAlg + "_global-router";
...
function [topologies, lineNames] = getTopologies(trustedNodeAlg)
    ...
    fiberOpts(1).L = 30 / sqrt(2) / (3 - 1);
    fiberOpts(2).L = 30 / sqrt(2) / (5 - 1);
    fiberOpts(3).L = 30 / sqrt(2) / (5 - 1);
    topologies = [
