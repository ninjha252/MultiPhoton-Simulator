...
% generatePlot("e91-burst", 'L', "key", 1:2:250);
...
%plotE91Burst();
%plotThreeStageBurst();
plotDecoyStateProfile();
plotThreeStageProfile();
plotE91Profile();
% plotTopologyProfiles();
...
function generateTopologyImages()
    ...
    scenarios = ["direct", "line_all_trusted", "square_all_trusted", "ring_all_trusted", "star_all_trusted"];
    fileNames = ["topo_direct", "topo_line", "topo_grid", "topo_ring", "topo_star"];
    ...
    for i = 1:numel(scenarios)
        ...
        exportgraphics(gca, "figures/topologies/" + fileName + ".png");
    end
end

function plotDirect()
    data = readtable("results/topologies/direct_L_100000.csv");

    X = 1:2:250;
    numX = numel(X);
    plot(X, data{1:numX, "Decoy"}, X, data{1:numX, "Three_Stage"}, X, data{1:numX, "E91"}, X, data{1:numX, "E91_Repeater"});
    % title("Direct Comparison");
    xlabel("Distance A - B");
    ylabel("Key Rate");
    legend(["Decoy", "Three-Stage", "E91 (No Repeater)", "E91"]);
    styleGraphPlot();
    
    
    fileName = "direct_L_100000";
    pause(1);
    exportgraphics(gcf,"figures/topologies/" + fileName + ".png");
end

function plotDecoyStateProfile()
    data = readtable("results/topologies/decoy-profile_L_100000.csv");

    X = 1:2:150;
    numX = numel(X);
    plot(X, data{1:numX, "Line"}, X, data{1:numX, "Square"}, X, data{1:numX, "Ring"}, X, data{1:numX, "Star"});
    % title("Decoy Profile");
    xlabel("Distance A - B");
    ylabel("Key Rate");
    legend(["Line", "Grid", "Ring", "Star"]);
    styleGraphPlot();
    
    
    fileName = "decoy-profile_L_100000";
    pause(1);
    exportgraphics(gcf,"figures/topologies/" + fileName + ".png");
end

function plotThreeStageProfile()
    data = readtable("results/topologies/3-stage-profile_L_100000.csv");

    X = 1:2:150;
    numX = numel(X);
    plot(X, data{1:numX, "Line"}, X, data{1:numX, "Square"}, X, data{1:numX, "Ring"}, X, data{1:numX, "Star"});
    % title("Three-Stage Profile");
    xlabel("Distance A - B");
    ylabel("Key Rate");
    legend(["Line", "Grid", "Ring", "Star"]);
    styleGraphPlot();
    ...
    fileName = "3-stage-profile_L_100000";
    pause(1);
    exportgraphics(gcf,"figures/topologies/" + fileName + ".png");
end
...
function plotE91Profile()
    data = readtable("results/topologies/e91-profile_L_100000.csv");
    ...
    fileName = "e91_L_100000";
    pause(1);
    exportgraphics(gcf,"figures/topologies/" + fileName + ".png");
end

function plotThreeStageBurst()
    data = readtable("results/topologies/3-stage-burst_L.csv");
    X = 1:80;
    numX = numel(X);
    plot(X, data{1:numX, "Burst1"}, X, data{1:numX, "Burst5"}, X, data{1:numX, "Burst10"}, X, data{1:numX, "Burst15"}, X, data{1:numX, "Burst20"});
    % title("Three-Stage Burst");
    xlabel("Distance A - B");
    ylabel("Key Rate");
    legend(["Burst = 1", "Burst = 5", "Burst = 10", "Burst = 15", "Burst = 20"]);
    styleGraphPlot();
    
    
    fileName = "3-stage-burst_L_100000";
    pause(1);
    exportgraphics(gcf,"figures/topologies/" + fileName + ".png");
end

function plotE91Burst()
    data = readtable("results/topologies/e91-burst_L_100000.csv");
    X = 1:2:250;
    numX = numel(X);
    plot(X, data{1:numX, "Burst1"}, X, data{1:numX, "Burst5"}, X, data{1:numX, "Burst10"}, X, data{1:numX, "Burst15"}, X, data{1:numX, "Burst20"});
    % title("Three-Stage Burst");
    ...
    fileName = "e91-burst_L_100000";
    pause(1);
    exportgraphics(gcf,"figures/topologies/" + fileName + ".png");
end


function plotTopologyProfiles()
    threeStageProfile = readtable("results/topologies/3-stage-profile_L_100000.csv");
    decoyProfile = readtable("results/topologies/decoy-profile_L_100000.csv");
    e91Profile = readtable("results/topologies/e91-profile_L_100000.csv");
    
    X = 1:2:150;
    numX = numel(X);
    topos = ["Line", "Square", "Ring", "Star"];
    for i = 1:numel(topos)
        topology = topos(i);
        figure(i);
        plot(X, decoyProfile{1:numX, topology}, X, threeStageProfile{1:numX, topology}, X, e91Profile{1:numX, topology});
        ...
        % title(topology + " Topology Profile");
        xlabel("Distance A - B");
        ylabel("Key Rate");
        legend(["Decoy", "Three-Stage", "E91"]);
        styleGraphPlot();
        
        fileName = lower(topology) + "-profile";
        pause(1);
        exportgraphics(gcf,"figures/topologies/" + fileName + ".png");
    end
end
