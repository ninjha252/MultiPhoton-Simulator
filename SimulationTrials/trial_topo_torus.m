...
%generatePlot("e91-burst", 'L', "key", 1:2:100);
%generatePlot("3-stage-profile", 'L', "all", 1:2:100);
% generatePlot("decoy-profile", 'L', "key", 1:2:150);


% generatePlot("3-stage-profile", 'L', "key", L);
...
function generateTopologyImages()
    %scenarios = ["direct", "line_all_trusted", "line", "square_all_trusted", "square", "ring_all_trusted", "ring", "star_all_trusted", "star", "Torus"];
    scenarios = "Torus";
    ...
    for i = 1:numel(scenarios)
        ...
        topo.plot();
        fileName = scenarioKey + "_topo_new";
        pause(1);
...
function xName = getXName(scenarioKey, xKey)
    if (scenarioKey == "direct" || scenarioKey == "e91-profile" || scenarioKey == "3-stage-profile" || scenarioKey == "decoy-profile" || scenarioKey == "3-stage-burst" || scenarioKey == "e91-burst" || scenarioKey == "3-stage-burst-torus")
        xName = "Distance A - B";
    else
        xName = xKey;
    end
...
function [scenarioNames, scenarioKeys] = getScenarioName(scenarioKey)
    ...
    lookup("3-stage-burst-torus") = "3-Stage Burst Torus";
...
function lineName = getLineName(scenarioKey, lineIdx, ~)
    ...
    elseif (scenarioKey == "3-stage-burst" || scenarioKey == "e91-burst"|| scenarioKey == "3-stage-burst-torus")
        lineName = "Burst = " + networkOpts.BURST;
    elseif (scenarioKey == "e91-profile" || scenarioKey == "3-stage-profile" || scenarioKey == "decoy-profile")
        lines = ["Torus","line", "Square", "Star", "ring"];
...
function [lineStyles, lineCount] = getLineStyles(scenarioKey, ~)
    if (scenarioKey == "direct")
        ...
        lineCount = 4;
        return;
    elseif (scenarioKey == "3-stage-burst" || scenarioKey == "e91-burst"|| "3-stage-burst-torus")
        % lineStyles = [ "-->", "--x", "--v"];
        % lineStyles = ["-", "--", ":"];
        lineStyles = [];
        lineCount = 5;
...
function networkOpts = getNetworkOpts(scenarioKey, lineIdx, xKey, xValue)
    ...
    if (scenarioKey == "3-stage-burst")
        ...
        burstValues = [50, 80, 100, 150, 200];
        networkOpts.BURST = burstValues(lineIdx);
    elseif (scenarioKey == "3-stage-burst-torus")
        networkOpts.type = protocols(2);
...
function topology = getTopology(scenarioKey, lineIdx, xKey, xValue)
    ...
    elseif (scenarioKey == "3-stage-burst")
         topology = TorusTopology(30, 30, "all", topologyOpts);
    elseif (scenarioKey == "3-stage-burst-torus")
        topology = TorusTopology(30, 30, "all", topologyOpts);
    elseif (scenarioKey == "e91-burst")
         % Single repeater divides distance
         topologyOpts.L = xValue;
         topology = TorusTopology(3, 3, "all", topologyOpts);
    elseif (scenarioKey == "e91-profile" || scenarioKey == "3-stage-profile" || scenarioKey == "decoy-profile")
        ...
        elseif (lineIdx ==1)
            tnAlg = "all";
            topologyOpts.L = xValue;
            topology = TorusTopology(TOR_DIMx, TOR_DIMY, tnAlg, topologyOpts);
        else
            error("Unsupported lineIdx '" + lineIdx + "'");
        end

    elseif(scenarioKey=="Torus")
        topologyOpts.L = xValue;
        topology = TorusTopology(TOR_DIMx, TOR_DIMY, "all", topologyOpts);
