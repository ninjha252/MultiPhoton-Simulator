classdef RingTopology < Topology
    ...
    methods
        function obj = RingTopology(totalNodes, trustedNodesAlg, fiberOpts)
            ...
            for node = 1:totalNodes
                ...
                x(node) = radius * sin(theta * node);
                y(node) = radius * cos(theta * node);
        ...
        function result = getName(obj)
            result = "T-" + obj.TrustedNodesAlg + "_" + string(length(obj.Nodes)) + "-node-ring";
        end
     ...
     methods(Access=private)
         function result = getTrustedNodesForAlg(obj, trustedNodesAlg, totalNodes)
             halfway = round((totalNodes + 1) / 2);
             if (trustedNodesAlg == "2-corners")
                 result = [1, halfway];
             elseif (trustedNodesAlg == "4-corners")
                 result = [1, round(halfway / 2), round(halfway), round(3 * halfway / 2)];  
